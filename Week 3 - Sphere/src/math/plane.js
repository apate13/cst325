...
Plane.prototype = {
  //----------------------------------------------------------------------------- 
	raycast: function(ray) {
		let numerator = this.normal.dot(this.validPoint) - (this.normal.dot(ray.origin));
		let denominator = this.normal.dot(ray.direction);

		let alpha = numerator / denominator;

		if (alpha > 0 && this.normal.dot(ray.direction) < 0) {
			let hitPoint = ray.origin.clone().add(ray.direction.clone().multiplyScalar(alpha));
			return {
				hit: true,
				point: hitPoint,
				normal: this.normal,
				distance: (alpha + alpha) / 2 
			};
		} else {
			return { hit: false }
		}
